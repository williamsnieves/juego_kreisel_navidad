<?php
	...
	$app->get('/toys/search/:term', 'searchToys');
	...
	function searchToys($term){
		...
		$sql_query = "SELECT id_product, short_desc, image, image2, image3, image4, image5, active FROM products where short_desc LIKE '%".$term."%' AND active = 1 ORDER BY short_desc ASC limit $per_page";
		//die($sql_query);
		//$sql_query = "SELECT id_producto, desc_corta , imagen_peq FROM productos limit $start, $per_page";
	...
	function getToys($page){
		...
		$sql_query = "SELECT id_product, short_desc, image, image2, image3, image4, image5, active FROM products where image != '' AND active = 1 ORDER BY short_desc ASC limit $start, $per_page";
		//die($sql_query);
		//$sql_query = "SELECT id_producto, desc_corta , imagen_peq FROM productos limit $start, $per_page";
	...
	function addCommitments(){
		...
		$sql = "INSERT INTO commitment_selected (`compromiso`,`custom_compromiso`,`email_parent_user`,`email_user`) VALUES ('".utf8_decode($data['commitment']['commitment_selected'])."','".utf8_decode($data['commitment']['custom_commitment_selected'])."','".$data['commitment']['email_parent']."','".$data['commitment']['email']."')";
	...
	function sendLetter(){
		...
		//$mail->AddCC('info@kreisel.com', 'Kreisel');
		$mail->AddCC('willians.nieves28@gmail.com', 'Kreisel');
		...
		<body>
			...
			<table background='http://www.kreisel.com/cartadenavidad/assets/fondostart.png' style='border:2px solid white;height: 600px;width: 800px;margin-left: auto;margin-right: auto; position:relative;'>
			        <tr>
			        	<td>
			        		 <table align='center' background='http://www.kreisel.com/cartadenavidad/assets/bgletter.png' style='width: 637px;height: 572px;'>
			        			<tr>
			        				<td style='padding:9em 3em 0 3em'>
			        					<p style='color: #328968;font-size: 1.3em;line-height: 1.2em;text-align: justify;'>Mi nombre  es $name, este año me porté $answer1 ,$answer2 hice todas mis tareas, $answer3 me comí toda mi comida y en el año 2015 me comprometo a $compromiso y a $custom_compromiso. Quisiera recibir estos juguetes en Navidad :</p>
										<div class='list_toys' style='width: 267px;height: 180px;padding-bottom:-19em; margin-bottom:-19em; padding-left:0.5em;margin-left:0.5em;'>
	...
	function getConnectionProductionLetter(){
		try {
	        ...
	        /*$db_username = "root";
	        $db_password = "";*/
	        $conn2 = new PDO('mysql:host=localhost;dbname=kreis1_cartanavidad', $db_username, $db_password);
	        $conn2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
